...
const gravity = 0.08;   // <--- 調整: 大幅降低重力，減少下墜
const damping = 0.95;  // <--- 調整: 稍微提高阻尼，讓運動更平滑但不會太快停止
  ...
  for (let nodes of chains) {
    ...
    for (let j = 1; j < nodes.length; j++) {
      ...
      if (mouseIsPressed || (touches.length > 0)) { 
        let currentMouseX = mouseIsPressed ? mouseX : touches[0].x;
        let currentMouseY = mouseIsPressed ? mouseY : touches[0].y;
        let d = dist(currentMouseX, currentMouseY, n.pos.x, n.pos.y);
        // <--- 調整: 增大推力從 15 到 30，讓珠子被推開得更快
        if (d < 150) n.pos.add(p5.Vector.sub(n.pos, createVector(currentMouseX, currentMouseY)).normalize().mult(30)); 
      }
    }
    // <--- 調整: 增加約束迭代次數從 2 到 4，使鏈條更快恢復穩定和形變後更強韌
