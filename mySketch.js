...
function preload() {
  // 圖片路徑已改為相對路徑，假設圖片都在 assets 資料夾內
...
function initCurtain() {
  ...
  // === 修正點 2: 「陸禹垛」文字居中 ===
  // 調整文字大小和Y座標，使其在視覺上更居中
  pg.textSize(width * 0.06); // 略微縮小文字大小以適應居中，並留出空間
  pg.text("陸  禹  垛", width / 2, height / 2); // 直接設置為畫布中心
  
  pg.loadPixels();
  for (let i = 0; i < cols; i++) {
    // 修正點 6: 確保 x 座標不會超出 pg 畫布的寬度
    let x = (i + 1) * spacingX;
    x = constrain(x, 0, width - 1); // 限制 x 在 [0, width-1] 範圍內

    let nodes = [];
    for (let j = 0; j < beadsPerChain; j++) {
      // 修正點 6: 確保 y 座標不會超出 pg 畫布的高度
      let y = j * spacingY;
      y = constrain(y, 0, height - 1); // 限制 y 在 [0, height-1] 範圍內
...
function drawExhibitionText() {
  ...
  // === 修正點 3: 右下角文字往上調整 ===
  textSize(width * 0.018); // 保持與左側文字大小一致
  textAlign(RIGHT, TOP); // 從頂部對齊
  let rightTxt = "三/二三-四/七";
  // 調整起始 Y 座標，使其更高。這個 0.55 可以根據實際效果微調
...
// === 優化後的 drawCurtain() 函數 (與上次提供的一致，主要用於渲染優化) ===
function drawCurtain() {
  for (let nodes of chains) {
    ...
    for (let j = 1; j < nodes.length; j++) {
      ...
      // 觸控點影響力 (維持不變)
      // 注意：這裡只檢查 mouseIsPressed，如果想同時響應觸摸屏，需要加上 touches.length > 0
      if (mouseIsPressed || (touches.length > 0)) { // 確保觸控也生效
    ...
    for (let n of nodes) {
      if (n.isText) {
        ellipse(n.pos.x, n.pos.y, 6.5, 6.5);
      }
    }
    pop(); 
  }

  // 底部文字 (維持不變，但調整為與右側文字類似的相對大小)
  fill(255, 200); noStroke(); 
  textSize(width * 0.02); // 保持與問題UI文字大小接近
...
function mousePressed() {
  // 珠簾撥動邏輯處理在 drawCurtain 裡，這裡只處理場景切換和按鈕點擊
  handleInteraction();
}
...
function shuffleCurrentOptions() {
  if (!questions[currentQuestion]) return; // 避免在 currentQuestion 無效時出錯
...
function windowResized() { 
  ...
  initCurtain(); 
  initBackgroundBeads(); // 視窗大小改變時重置背景珠子
}